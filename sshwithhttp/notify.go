...
// 监控客户端请求的目录，返回事件
func (w *WatchMgr) WatchFile(file File, stream pb.WatchMgr_WatchFileServer) (err error) {
...
func (w *WatchMgr) WatchFileAndSync(file File, stream pb.WatchMgr_WatchFileAndSyncServer) (err error) {
	log.Infof("client watch path:%s, recursive:%t", file.Name, file.Recursive)
	// 判断路径是否存在
	if ok, _ := PathExists(file.Name); !ok {
		log.Errorf("[%s] not exist", file.Name)
